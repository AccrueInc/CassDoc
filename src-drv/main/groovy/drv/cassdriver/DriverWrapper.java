...
public class DriverWrapper
{
  ...
  public boolean isHealthyQUORUM()
  {
    ...
    try {
      ...
      log.warn("healthcheck db statement failed: " + e.getMessage(), e);
      return false;
    }
  }
      ...
      synchronized (initLock) {
        ...
        if (!initialized) {
          try {
            ...
            log.error("initDataSourceFailed: " + e.getMessage() + "; not stopping context initialization; hopefully DB will be available later", e);
            if (strict) { throw e; }
          }
        }
      }
  ...
  private void doInitDataSources()
  {
    ...
    if (enableCompression) {
      log.info("CASSANDRA JAVA-DRIVER INIT: enable compression");
      builder = builder.withCompression(ProtocolOptions.Compression.LZ4);
    }

    if (useConstantTimeReconnectionPolicy) {
      log.info("CASSANDRA JAVA-DRIVER INIT: constant reconnection policy: millis: {}", constantTimeReconnectionDelayMilliseconds);
    ...
    if (useDowngradingConsistencyRetryPolicy) {
      log.info("CASSANDRA JAVA-DRIVER INIT: downgrading consistency retry policy");
    ...
    log.info("CASSANDRA JAVA-DRIVER INIT: Connected to cluster: {}\n", metadata.getClusterName());
    for (Host host : metadata.getAllHosts()) {
      log.info("CASSANDRA JAVA-DRIVER INIT: Datacenter: {}; Host: {}; Rack: {}; Status: {}\n", host.getDatacenter(), host.getAddress(), host.getRack(), host.isUp() ? "up" : "down");
    }
  }
  ...
  public Session getSession()
  {
    ...
    if (!initialized) { throw new IllegalStateException("database connection not initialized"); }
  ...
  {
    if (destroyed) { throw new IllegalStateException("application is shutting down"); }
  }
  ...
  public Statement prepare(final String keyspace, final String cql, final Object[] prepArgsIn, final String consistency, Long usingTimestamp)
  {
    if (prepArgsIn != null && prepArgsIn.length != 0) {
      ...
      BoundStatement bindStmt = prepStmt.bind(prepArgsIn);
  ...
  public ResultSet executeSyncStatementBatch(List<St> statements, final String consistency, final Long usingTimestamp)
  {
    ...
    StringBuilder batchcql = log.isDebugEnabled() ? new StringBuilder() : null;
    ...
    for (St stmt : statements) {
      if (keyspace == null) {
        ...
        if (!StringUtils.equals(keyspace, stmt.getKeyspace())) {
          log.warn("batch statement keyspace mismatch: {} {}", keyspace, stmt.getKeyspace());
        }
      }
    ...
    log.debug(log.isDebugEnabled() ? "Batch\n " + batchcql : null);
  ...
  private void logClusterStatus()
  {
    try {
      for (Host host : cluster.getMetadata().getAllHosts()) {
        log.info("{} is {}", host, host.isUp() ? "up" : "down");
      }
    } catch (RuntimeException re) {
      log.warn("get cluster metadata failed: " + re.getMessage(), re);
    }
  }
  ...
  public void executeQueryCQL(final String keyspace, String cql, Object[] prepArgs, final RowCallbackHandler rch, final String consistency)
  {
    ...
    for (Row curRow : cassRS) {
      ...
      try {
        ...
        log.warn("Query limit threshold reached for query '{}' returning partial search results", cql);
        break;
    ...
    log.debug("query: {}; args: {}; count: {}", cql, prepArgs, count);
  ...
  public Set<String> getKeyspaces()
  {
    ...
    log.debug("getKeyspaces: return: {}", keyspaceNames);
