...
class GetAttrCmd extends QueryCmd {
  ...
  GetAttrRCH queryCassandra(CommandExecServices svcs, OperationContext opctx, Detail detail, Object... args) {
    ...
    StringBuilder cql = new StringBuilder(64)
    cql << "SELECT e,p,d,t,zv"
    if (detail.attrWritetimeMeta != null) { rch.writetimeCol = true; cql << ",writetime(" << detail.attrWritetimeMeta << ")" }
    if (detail.attrTokenMeta) { rch.tokenCol = true; cql << ",token(e)" }
    if (detail.attrMetaIDMeta || detail.attrMetaDataMeta) { rch.attrMetaCol = true; cql << ",z_md" }
...
class GetDocRCH implements RowCallbackHandler {
  ...
  public void processRow(Row row) throws CQLException {
    ...
    if (writetimeCol) { writetime = row.getLong(3) }
    if (tokenCol) { token = row.partitionKeyToken.value.toString() }
    if (metaCol) { metadata_id = row.getString("z_md") }
  }
}
